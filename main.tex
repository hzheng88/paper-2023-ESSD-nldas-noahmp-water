...
The energy budget terms consist of two components.
\begin{align}
    S = & S_{can} + S_{gnd} \text{,} \\
    I = & I_{can} + I_{gnd} \text{,} \\
    H = & H_{can} + H_{gnd} \text{,} \\
...
$\rho_{ice} = 916.2$ \unit{kg~m^{-3}} is the density of ice. $w_{ice,i}$
...
$\rho_{air}$ (\unit{m}) is the density of air. $c_{mine}$
(\unit{J~m^{-3}~K^{-1}}) is the volumetric heat capacity of dry soil matter, the
value of which is $2\times10^6$ in Noah-MP.
