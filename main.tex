...
using the NLDAS\nobreakdash-2 downward solar (\(S_u\), \si{W~m^{-2}}) and
...
gates of the gas diffusion of CO\textsubscript{2}, H\textsubscript{2}O, and
O\textsubscript{2} between the internal leaf structure and external atmosphere.
The opening and closing of the stomata is characterized by stomatal conductance
in LSMs.
\begin{align}
...
\end{align}
where \(g_{s,CO_2}\) (\si{\umol_{CO_2}~m^{-2}~s^{-1}}), \(g_{s,H_2O}\)
(\si{\umol_{H_2O}~m^{-2}~s^{-1}}), and \(g_{s,O_2}\)
(\si{\umol_{O_2}~m^{-2}~s^{-1}}) are the stomatal conductance for
CO\textsubscript{2}, H\textsubscript{2}O, and O\textsubscript{2}, respectively.
\(A_{CO_2}\) (\si{umol_{CO_2}~m^{-2}~s^{-1}}), \(A_{H_2O}\)
(\si{umol_{H_2O}~m^{-2}~s^{-1}}), and \(A_{O_2}\)
(\si{umol_{O_2}~m^{-2}~s^{-1}}) are the fluxes of CO\textsubscript{2},
H\textsubscript{2}O, and O\textsubscript{2}. \(P_{atm}\) is the atmospheric
pressure (\si{Pa}). \(p_{CO_2}\), \(p_{H_2O}\), and \(p_{O_2}\) are the partial
pressure (\si{Pa}) of CO\textsubscript{2}, H\textsubscript{2}O, and
O\textsubscript{2}, where the subscript \(s\) denotes the pressure at the leaf
surface, and the subscript \(i\) denotes the pressure inside the leaf. The water
vapor pressure inside the leaf (\(p_{H_2O,i}\)) is assumed to be saturated,
which is a function of leaf temperature (\(T_l\), \si{K}).
...
follows:
\begin{equation}
  g_{s,H_2O} = m A_{CO_2} \frac{P_{atm}}{p_{CO_2,s}}\frac{p_{H_2O,s}}{p_{H_2O,i}} + b \text{.}
\end{equation}
There are two calibratable parameters: (1) \(m\), a vegetation-type dependent
parameter (\si{\umol_{H_2O}~\umol_{CO_2}^{-1}}), and (2) \(b\), the stomatal
conductance at zero photosynthesis (\si{\umol_{CO2}~m^{-2}~s^{-1}}). In Noah-MP,
\(m\) and \(b\) are assigned using a look-up table of vegetation types. The
typical value of \(m\) is 9 for C\textsubscript{3} plants and 6 for
C\textsubscript{4} plants. The typical value of \(b\) is
\si{5\times10^{-5}~m~s^{-1}}.

The stomatal condutance of CO\textsubscript{2}, and O\textsubscript{2} are
propositional to that of H\textsubscript{2}O.
\begin{align}
  g_{s,CO_2} = & k_{CO_2,H_2O} g_{s,H_2O} \text{,} \\
  g_{s,O_2} = & k_{O_2,H_2O} g_{s,H_2O} \text{,}
\end{align}
where the values of the coefficients (i.e., \(k_{CO_2,H_2O}\), \(k_{O_2,H_2O}\))
vary with the flow condition. In flowing air, the diffusion rate of
different gases (e.g., CO\textsubscript{2}, H\textsubscript{2}O, and
O\textsubscript{2}) are the same. In still air, the diffusion rate of gas \(X\)
is inversely propositional to its molecular weight \(M_X\) according to the
Graham's law:
\begin{equation*}
  g_{s,X} \propto \frac{1}{\sqrt{M_X}} \text{.}
\end{equation*}
Under such conditions, the stomatal conductance for CO\textsubscript{2},
H\textsubscript{2}O, and O\textsubscript{2} are calculated as follows:
\begin{align}
  g_{s,CO_2} \approx & 0.64 g_{s,H_2O} \text{,} \\
  g_{s,O_2} \approx  & 0.75 g_{s,H_2O} \text{.}
\end{align}
A value between 1 and the equations have to be set considering the flow
conditions.

Note that two different units are used for stomatal conductance:
\si{\umol_{H_2O}~m^{-2}~s^{-1}} and \si{kg~m^{-2}~s^{-1}}. The conversion
between the two units follows \citet{cowan1978}.
