...
Leaf stomatal conductance (\(g_s\), \si{\mu mol~m^{-2}~s^{-1}}) is
\begin{equation}
    g_s = m\frac{A}{c_s}\frac{e_s}{e_i}P_{atm}+b \text{,}
\end{equation}
where \(m\) is a plant function type dependent parameter, \(A\) is leaf
photosynthesis (\si{\umol~m^{-2}~s^{-1}}), \(c_s\) is the CO\textsubscript{2}
partial pressure at the leaf surface (\si{Pa}), \(e_s\) is the water vapor
pressure at the leaf surface (\si{Pa}), \(e_i\) is the water vapor pressure
inside the stomata (\si{Pa}), \(P_{atm}\) is the ambient air pressure (\si{Pa}),
and \(b\) is the stomatal conductance at zero photosynthesis
(\si{\umol~m^{-2}~s^{-1}}).
