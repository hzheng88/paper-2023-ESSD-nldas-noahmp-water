...
Leaf stomata are the small pores typically on the underside of leaves. They are
the gates of the gas exchanges of CO\textsubscript{2}, H\textsubscript{2}O, and
...
formulates the stomatal conductance of H\textsubscript{2}O (\(g_s\),
\si{\umol_{H_2O}~m^{-2}~s^{-1}}) as follows:
\begin{equation}
  g_s = m A \frac{P_{atm}}{c_s}\frac{e_s}{e_i} + b \text{,}
\end{equation}
where \(A\) is the photosynthesis rate (\si{\umol_{CO_2}~m^{-2}~s^{-1}}),
\(c_s\) is the CO\textsubscript{2} partial pressure at the leaf surface
(\si{Pa}), \(e_s\) is the water vapor pressure at the leaf surface (\si{Pa}),
\(e_i\) is the saturation water vapor pressure inside the leaf (\si{Pa}) at the
leaf temperature (\(T_l\), \si{k}), \(P_{atm}\) is the ambient atmosphere
pressure (\si{Pa}).

The Ball-Berry scheme has two calibratable parameters: (1) \(m\), a
vegetation-type dependent parameter (\si{\umol_{H_2O}~\umol_{CO_2}^{-1}}), and
(2) \(b\), the stomatal conductance at zero photosynthesis
(\si{\umol_{CO2}~m^{-2}~s^{-1}}). In Noah-MP, \(m\) and \(b\) are assigned using
a look-up table of vegetation types. The typical value of \(m\) is 9 for
C\textsubscript{3} plants and 6 for C\textsubscript{4} plants. The typical value
of \(b\) is \si{5\times10^{-5}~m~s^{-1}}.
...
conductance from Noah \citep{chen2001MWR}. The Jarvis scheme calculates the
stomatal conductance (\(g_s\), \si{m~s^{-1}}) as follows:
\begin{align}
  g_s =   & g_{s,max} f_{S} f_{D} f_{T} \beta \text{,} \\
  f_{S} = & \frac{\frac{g_{s,min}}{g_{s,max}}+f_{s}}{1+f_{s}} \text{,} \\
  f_{s} = & 0.55 \frac{S_d}{S_{gl}} \frac{2}{l_a} \text{,} \\
