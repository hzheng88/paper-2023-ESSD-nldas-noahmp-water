...
variables are provided for each 1/8° NLDAS\nobreakdash-2 grid indicated by the
...
z_{soil,3}=0.6\)\,\unit{m}, and \(\Delta z_{soil,4}=1.0\)\,\unit{m} are the
...
(\(E_{tran}\)). \(L_{gnd}\) is the latent heat flux from the ground and
associated with ground evaporation (\(E_{gnd}\)). The energy budget terms of the
canopy and ground are both balanced:
\begin{align}
...
within the whole land column (\unit{J~m^{-2}~s^{-1}}). \(c_{snow,j}\) is the
...
(\(C_{ice} = 2.108 \times 10^3\)\,\unit{J~kg^{-1}~K^{-1}}) and supercoolded
water (\(C_{wat} = 4.188 \times 10^3\)\,\unit{J~kg^{-1}~K^{-1}}) as follows:
\begin{equation}
    c_{snow,j} = C_{ice} \rho_{ice} w_{ice,j}
    + C_{wat} \rho_{wat} w_{liq,j} \text{,}
\end{equation}
where \(\rho_{ice} = 916.2\)\,\unit{kg~m^{-3}} is the density of ice.
$w_{ice,j}$ is the volumetric snow ice content (\unit{m^3~m^{-3}}),
\(w_{liq,j}\) is the volumetric liquid water content (\unit{m^3~m^{-3}}).
\(c_{soil,i}\) is the volumetric heat capacity of the
\textit{i}\textsuperscript{th} soil layer (\unit{J~m^{-3}~K^{-1}}), which is a
mixture of dry soil matter, water, ice, and air.
\begin{multline}
...
\(\rho_{air}\) (\unit{m}) is the density of air. \(c_{mine}\) is the volumetric
heat capacity of dry soil matter (\unit{J~m^{-3}~K^{-1}}), whose value is
\(2\times10^6\) in Noah-MP. \(w_{ice,i}\) and \(w_{liq,i}\) are the volumetric
content of frozen and liquid water (\unit{m^3~m^{-3}}), respectively, and their
...
precipitaiton throughfall, snowmelt, and dewfall, \unit{kg~m^-2~s^{-1}}),
\(f_{sat}\) is the saturation fraction of the grid cell (\unit{m^2~m^{-2}}), and
\(z_{bot}\) is the depth of the soil column bottom (\unit{m}, positive down).
\(f_{frz,i}\) is the fractional frozen area of the
\textit{i}\textsuperscript{th} soil layer (\unit{m^2~m^{-2}}). Its
...
saturation area fraction (\unit{m^2~m^{-2}}), which is defined as the cumulative
...
(\unit{kg~m^{-2}~s^{-1}}); and (4) \(\Lambda\), the grid cell mean topographic
index. In this study, the parameters have the following values: \(f_{sat,max} =
0.38\)\,\unit{m^2~m^{-2}}, $f=6$, $R_{sub,max}=5$\,\unit{kg~m^{-2}~s^{-1}}, and
\(\Lambda = 10.5\).
...
subsurface runoff: \(f=2\), and \(R_{sub,max}=4\)\,\unit{kg~m^{-2}~s^{-1}}.
...
(\unit{kg~m^{-2}~s^{-1}}). The infiltraiton is derived from the approximate
...
(kg\,m$^{-2}$), \(w_d\) is the water deficit of the soil column
(\unit{kg~m^{-2}}), and \(\Delta t\) is the model time step (\unit{s}).
...
\(K_{\Delta t,ref} = \frac{3}{86400}\)\,\unit{s^{-1}}, and \(k_{ref}=2 \times
10^{-3}\)\,\unit{kg~m^{-2}~s^{-1}}.
...
(i.e., \(i=N_{soil}\)) of the soil (\unit{kg~m^{-2}~s^{-1}}).
...
Leaf stomatal conductance (\(g_s\), \unit{\mu mol~m^{-2}~s^{-1}}) is
\begin{equation}
    g_s = m\frac{A}{c_s}\frac{e_s}{e_i}P_{atm}+b \text{,}
\end{equation}
where \(m\) is a plant function type dependent parameter, \(A\) is leaf
photosynthesis (\unit{\mu mol~m^{-2}~s^{-1}}), \(c_s\) is the
CO\textsubscript{2} partial pressure at the leaf surface (\unit{Pa}), \(e_s\) is
the water vapor pressure at the leaf surface (\unit{Pa}), \(e_i\) is the water
vapor pressure inside the stomata (\unit{Pa}), \(P_{atm}\) is the ambient air
pressure (\unit{Pa}), and \(b\) is the stomatal conductance at zero
photosynthesis (\unit{\mu mol~m^{-2}~s^{-1}}).
...
\begin{align}
    R_c = & R_{c,min} \frac{1}{l_a f_1 f_2 f_3 f_4} \text{,} \\
    f_1 = & \frac{\frac{R_{c,min}}{R_{c,max}}+f}{1+f} text{,} \\
    f =   & 0.55 \frac{R_g}{R_{gl}} \frac{2}{l_a} \text{,} \\
    f_2 = & \frac{1}{1+h_s[q_{sat}(T_l)-q_a]} \text{,} \\
    f_3 = & 1- 0.0016 (T_{ref} - T_l)^2 \text{,} \\
    f_4 = & \sum_{i=1}^{3} \frac{(\theta_i-\theta_w)d_z}{(\theta_{ref}-\theta_w)(d+d)} \text{,} \\
\end{align}
